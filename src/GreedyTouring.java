...
public class GreedyTouring {
  ...
  private boolean endOfTheDay(Site currentSite, int currentTime) {
    for (Site possibleSite : unvisitedSites) {
      if (couldVisit(currentSite, possibleSite, currentTime)) {
        return false;
      }
    }
    return true;
  ...
  private Site findNearestOpenSite(Site currentSite, int currentTime) {
    ...
    for (Site possibleSite : unvisitedSites) {
      ...
      if (totalDistance < closestDistance && 
          couldVisit(currentSite, possibleSite, currentTime)) {
  ...
  private boolean couldVisit(Site currentSite, Site possibleSite, int currentTime) {
    ...
    if (currentTime > possibleSite.closingHour[currentDay]
        || (currentTime + possibleSite.desiredTime) >= possibleSite.closingHour[currentDay]) {
      return false;
    } else {
      return true;
    }
